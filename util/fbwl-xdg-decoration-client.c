...
struct fbwl_xdg_decoration_app {
...
static int create_shm_fd(size_t size) {
    int fd = -1;
#ifdef MFD_CLOEXEC
    fd = memfd_create("fbwl-xdg-decoration-client", MFD_CLOEXEC);
    if (fd >= 0) {
    ...
    char template[] = "/tmp/fbwl-xdg-decoration-client-shm-XXXXXX";
    fd = mkstemp(template);
    if (fd < 0) {
        return -1;
    }
...
static void xdg_surface_handle_configure(void *data, struct xdg_surface *xdg_surface,
        uint32_t serial) {
    struct fbwl_xdg_decoration_app *app = data;
    xdg_surface_ack_configure(xdg_surface, serial);
    app->xdg_configured = true;
...
static void decoration_handle_configure(void *data,
    ...
    app->got_mode = true;
    app->mode = mode;
    app->ok = (mode == ZXDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE);
    if (app->ok) {
        printf("ok xdg_decoration server_side\n");
        fflush(stdout);
    } else {
        fprintf(stderr, "fbwl-xdg-decoration-client: unexpected mode=%u\n", mode);
    }
}
...
static void cleanup(struct fbwl_xdg_decoration_app *app) {
...
int main(int argc, char **argv) {
    ...
    struct fbwl_xdg_decoration_app app = {0};
    app.current_width = 64;
    app.current_height = 64;

    app.display = wl_display_connect(socket_name);
    if (app.display == NULL) {
        fprintf(stderr, "fbwl-xdg-decoration-client: wl_display_connect failed\n");
        return 1;
    }
    ...
    if (app.compositor == NULL || app.shm == NULL || app.wm_base == NULL || app.decoration_mgr == NULL) {
        fprintf(stderr, "fbwl-xdg-decoration-client: missing required globals\n");
        cleanup(&app);
        return 1;
    }
    ...
    if (app.surface == NULL) {
        fprintf(stderr, "fbwl-xdg-decoration-client: wl_compositor_create_surface failed\n");
        cleanup(&app);
        return 1;
    }
    ...
    if (app.xdg_surface == NULL) {
        fprintf(stderr, "fbwl-xdg-decoration-client: xdg_wm_base_get_xdg_surface failed\n");
        cleanup(&app);
        return 1;
    }
    ...
    xdg_toplevel_set_title(app.xdg_toplevel, "fbwl-xdg-decoration-client");

    app.decoration = zxdg_decoration_manager_v1_get_toplevel_decoration(app.decoration_mgr,
        app.xdg_toplevel);
    if (app.decoration == NULL) {
        fprintf(stderr, "fbwl-xdg-decoration-client: get_toplevel_decoration failed\n");
        cleanup(&app);
        return 1;
    }
    ...
    while (!app.got_mode && now_ms() < deadline) {
        ...
        if (ret < 0) {
            ...
            fprintf(stderr, "fbwl-xdg-decoration-client: poll failed: %s\n", strerror(errno));
            cleanup(&app);
            return 1;
        }
        if (pfd.revents & (POLLERR | POLLHUP)) {
            fprintf(stderr, "fbwl-xdg-decoration-client: compositor hung up\n");
            cleanup(&app);
            return 1;
        }
        if (pfd.revents & POLLIN) {
            if (wl_display_dispatch(app.display) < 0) {
                fprintf(stderr, "fbwl-xdg-decoration-client: wl_display_dispatch failed: %s\n",
                    strerror(errno));
                cleanup(&app);
                return 1;
            }
    ...
    if (!app.got_mode) {
        fprintf(stderr, "fbwl-xdg-decoration-client: timed out waiting for configure\n");
        cleanup(&app);
        return 1;
    }
    if (!app.ok) {
