...
static int create_shm_fd(size_t size) {
    int fd = -1;
#ifdef MFD_CLOEXEC
    fd = memfd_create("fbwl-clipboard-client", MFD_CLOEXEC);
    if (fd >= 0) {
    ...
    char template[] = "/tmp/fbwl-clipboard-client-shm-XXXXXX";
    fd = mkstemp(template);
    if (fd < 0) {
        return -1;
    }
...
static void try_set_selection(struct fbwl_clipboard_app *app) {
    if (app == NULL || !app->mode_set || app->selection_set) {
        return;
    }
    if (!app->have_serial || app->data_device == NULL || app->data_source == NULL) {
        return;
    }
...
static void seat_handle_capabilities(void *data, struct wl_seat *seat, uint32_t caps) {
    struct fbwl_clipboard_app *app = data;
...
static void data_offer_handle_action(void *data, struct wl_data_offer *offer, uint32_t action) {
    (void)data;
    (void)offer;
    (void)action;
}
...
static void registry_handle_global(void *data, struct wl_registry *registry,
    ...
    if (strcmp(interface, wl_data_device_manager_interface.name) == 0) {
        uint32_t v = version < 3 ? version : 3;
        app->data_device_mgr = wl_registry_bind(registry, name,
...
static void cleanup(struct fbwl_clipboard_app *app) {
    if (app->offer.offer != NULL) {
        wl_data_offer_destroy(app->offer.offer);
    }
...
static int receive_selection_text(struct fbwl_clipboard_app *app, char **out_text) {
    if (app == NULL || out_text == NULL) {
        return 1;
    }

    struct wl_data_offer *offer = app->selection_offer;
    if (offer == NULL) {
        return 1;
    }
    ...
    int pipefd[2];
    if (pipe(pipefd) != 0) {
        fprintf(stderr, "fbwl-clipboard-client: pipe failed: %s\n", strerror(errno));
        return 1;
    }
    ...
    for (;;) {
        ...
        if (prc == 0) {
            fprintf(stderr, "fbwl-clipboard-client: timed out waiting for clipboard data\n");
            free(buf);
            close(pipefd[0]);
            return 1;
        }
        if (prc < 0) {
            ...
            fprintf(stderr, "fbwl-clipboard-client: poll failed: %s\n", strerror(errno));
            free(buf);
            close(pipefd[0]);
            return 1;
        }
        ...
        if (n < 0) {
            ...
            fprintf(stderr, "fbwl-clipboard-client: read failed: %s\n", strerror(errno));
            free(buf);
            close(pipefd[0]);
            return 1;
        }
        ...
        if (new_buf == NULL) {
            fprintf(stderr, "fbwl-clipboard-client: OOM\n");
            free(buf);
            close(pipefd[0]);
            return 1;
        }
...
int main(int argc, char **argv) {
    ...
    struct fbwl_clipboard_app app = {0};
    app.display = wl_display_connect(socket_name);
    if (app.display == NULL) {
        fprintf(stderr, "fbwl-clipboard-client: wl_display_connect failed (WAYLAND_DISPLAY=%s): %s\n",
    ...
    if (wl_display_roundtrip(app.display) < 0) {
        fprintf(stderr, "fbwl-clipboard-client: wl_display_roundtrip failed: %s\n", strerror(errno));
        cleanup(&app);
        return 1;
    }
    ...
    if (app.compositor == NULL || app.shm == NULL || app.wm_base == NULL ||
        ...
        fprintf(stderr,
            "fbwl-clipboard-client: missing globals: compositor=%p shm=%p xdg_wm_base=%p seat=%p data_device_mgr=%p\n",
    ...
    if (app.data_device == NULL) {
        fprintf(stderr, "fbwl-clipboard-client: wl_data_device_manager_get_data_device failed\n");
        cleanup(&app);
        return 1;
    }
    ...
    if (app.mode_set) {
        app.data_source = wl_data_device_manager_create_data_source(app.data_device_mgr);
        if (app.data_source == NULL) {
            fprintf(stderr, "fbwl-clipboard-client: create_data_source failed\n");
            cleanup(&app);
            return 1;
        }
    ...
    if (app.surface == NULL) {
        fprintf(stderr, "fbwl-clipboard-client: wl_compositor_create_surface failed\n");
        cleanup(&app);
        return 1;
    }
    app.xdg_surface = xdg_wm_base_get_xdg_surface(app.wm_base, app.surface);
    if (app.xdg_surface == NULL) {
        fprintf(stderr, "fbwl-clipboard-client: xdg_wm_base_get_xdg_surface failed\n");
        cleanup(&app);
        return 1;
    }
    ...
    xdg_toplevel_set_title(app.xdg_toplevel, app.mode_set ? "fbwl-clipboard-set" : "fbwl-clipboard-get");
    ...
    while (!app.configured && now_ms() < deadline) {
        ...
        if (pfd.revents & (POLLERR | POLLHUP)) {
            fprintf(stderr, "fbwl-clipboard-client: compositor hung up\n");
            cleanup(&app);
            return 1;
        }
        if (pfd.revents & POLLIN) {
            if (wl_display_dispatch(app.display) < 0) {
                fprintf(stderr, "fbwl-clipboard-client: wl_display_dispatch failed: %s\n",
                    strerror(errno));
                cleanup(&app);
                return 1;
            }
    ...
    if (!app.configured) {
        fprintf(stderr, "fbwl-clipboard-client: timed out waiting for xdg_surface.configure\n");
        cleanup(&app);
        return 1;
    }
    ...
    if (app.mode_set) {
        ...
        if (!app.selection_set) {
            fprintf(stderr, "fbwl-clipboard-client: timed out waiting to set selection\n");
            cleanup(&app);
            return 1;
        }
    ...
    if (app.selection_offer == NULL) {
        fprintf(stderr, "fbwl-clipboard-client: timed out waiting for selection offer\n");
        cleanup(&app);
        return 1;
    }
