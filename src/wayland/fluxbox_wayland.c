...
static struct wlr_buffer *fbwl_text_buffer_create(const char *text, int width, int height,
...
#ifdef HAVE_SYSTEMD
static bool server_sni_start(struct fbwl_server *server, struct wl_event_loop *loop);
static void server_sni_finish(struct fbwl_server *server);
#endif
...
static void server_output_power_set_mode(struct wl_listener *listener, void *data) {
    ...
    if (server == NULL || event == NULL || event->output == NULL) {
        return;
    }
...
static struct fbwl_menu *fbwl_menu_create(const char *label) {
...
static bool fbwl_menu_add_exec(struct fbwl_menu *menu, const char *label, const char *cmd) {
...
static bool fbwl_menu_add_exit(struct fbwl_menu *menu, const char *label) {
    if (menu == NULL) {
        return false;
    }
    if (!fbwl_menu_reserve_items(menu, 1)) {
...
static bool fbwl_menu_add_view_action(struct fbwl_menu *menu, const char *label,
...
static bool fbwl_menu_add_submenu(struct fbwl_menu *menu, const char *label, struct fbwl_menu *submenu) {
...
static bool server_menu_parse_file_into(struct fbwl_server *server, const char *path,
        struct fbwl_menu **stack, size_t *depth) {
    if (server == NULL || path == NULL || *path == '\0' || stack == NULL || depth == NULL) {
        return false;
    }
...
static bool server_menu_load_file(struct fbwl_server *server, const char *path) {
    ...
    size_t depth = 0;
    stack[0] = server->root_menu;
    if (!server_menu_parse_file_into(server, path, stack, &depth)) {
        fbwl_menu_free(server->root_menu);
...
static struct wlr_buffer *fbwl_cairo_buffer_create(cairo_surface_t *surface) {
    if (surface == NULL) {
        return NULL;
    }
...
#ifdef HAVE_SYSTEMD
            if (server->sni.bus != NULL && ui->tray_services != NULL && ui->tray_paths != NULL) {
                const char *service = ui->tray_services[i];
                const char *path = ui->tray_paths[i];
                if (service != NULL && service[0] != '\0' && path != NULL && path[0] == '/') {
                    int r = sd_bus_call_method_async(server->sni.bus, NULL, service, path,
                        "org.kde.StatusNotifierItem", method, reply, server, "ii", lx, ly);
                    if (r < 0) {
                        wlr_log(WLR_INFO, "SNI: %s send failed id=%s: %s", action, id, strerror(-r));
                    } else {
                        wlr_log(WLR_INFO, "SNI: %s sent id=%s", action, id);
                    }
                }
            }
#endif
...
static void server_apps_rules_free(struct fbwl_server *server) {
    if (server == NULL || server->apps_rules == NULL) {
        return;
    }

    for (size_t i = 0; i < server->apps_rule_count; i++) {
        apps_rule_match_free(&server->apps_rules[i].app_id);
        apps_rule_match_free(&server->apps_rules[i].instance);
        apps_rule_match_free(&server->apps_rules[i].title);
    }

    free(server->apps_rules);
    server->apps_rules = NULL;
    server->apps_rule_count = 0;
}
...
static bool apps_rule_matches_view(const struct fbwl_apps_rule *rule, const struct fbwl_view *view) {
...
static const struct fbwl_apps_rule *server_apps_rules_match(struct fbwl_server *server,
        const struct fbwl_view *view, size_t *rule_index_out) {
    if (server == NULL || view == NULL || server->apps_rules == NULL || server->apps_rule_count == 0) {
        return NULL;
    }

    for (size_t i = 0; i < server->apps_rule_count; i++) {
        const struct fbwl_apps_rule *rule = &server->apps_rules[i];
        if (apps_rule_matches_view(rule, view)) {
...
static bool server_apps_rules_add(struct fbwl_server *server, struct fbwl_apps_rule *rule) {
    if (server == NULL || rule == NULL) {
        return false;
    }

    struct fbwl_apps_rule *rules =
        realloc(server->apps_rules, (server->apps_rule_count + 1) * sizeof(*rules));
    if (rules == NULL) {
        return false;
    }
    server->apps_rules = rules;
    server->apps_rules[server->apps_rule_count++] = *rule;
    memset(rule, 0, sizeof(*rule));
    return true;
}

static bool server_apps_rules_load_apps_file(struct fbwl_server *server, const char *path) {
    if (server == NULL || path == NULL || *path == '\0') {
        return false;
    }
    ...
    size_t before = server->apps_rule_count;
    bool in_app = false;
    ...
    while ((n = getline(&line, &cap, f)) != -1) {
        ...
        if (strcasecmp(key, "end") == 0) {
            (void)server_apps_rules_add(server, &cur);
            in_app = false;
            continue;
        }
    ...
    wlr_log(WLR_INFO, "Apps: loaded %zu rules from %s", server->apps_rule_count - before, path);
    return true;
}
...
static void xdg_toplevel_map(struct wl_listener *listener, void *data) {
    ...
    if (!view->apps_rules_applied) {
        apps_rule = server_apps_rules_match(view->server, view, &apps_rule_idx);
        if (apps_rule != NULL) {
...
static void sni_item_destroy(struct fbwl_sni_item *item) {
    if (item == NULL) {
        return;
    }
...
static struct wlr_buffer *sni_icon_buffer_from_argb32(const uint8_t *argb, size_t len, int width, int height) {
...
static const char *sni_status_str(enum fbwl_sni_status status) {
    switch (status) {
...
static enum fbwl_sni_status sni_status_parse(const char *s) {
    if (s == NULL || s[0] == '\0') {
...
static struct wlr_buffer *sni_icon_compose_overlay(struct wlr_buffer *base, struct wlr_buffer *overlay) {
...
static void sni_item_update_render_icon(struct fbwl_sni_item *item) {
    ...
    if (item->status == FBWL_SNI_STATUS_PASSIVE) {
        ...
        if (item->watcher != NULL && item->watcher->server != NULL) {
            server_toolbar_ui_rebuild(item->watcher->server);
        }
        return;
    }
...
static char *sni_icon_find_png_in_data_dir(const char *base, const char *icon_name, bool has_png_ext,
    ...
    {
        int n = snprintf(NULL, 0, "%s/pixmaps/%s%s", base, icon_name, has_png_ext ? "" : ".png");
        if (n > 0) {
            size_t len = (size_t)n + 1;
            char *path = malloc(len);
            if (path != NULL) {
                snprintf(path, len, "%s/pixmaps/%s%s", base, icon_name, has_png_ext ? "" : ".png");
                if (sni_path_is_regular_file_limited(path)) {
                    return path;
                }
                free(path);
...
static char *sni_icon_find_png_in_icon_dir(const char *base, const char *icon_name, bool has_png_ext,
    ...
    {
        int n = snprintf(NULL, 0, "%s/%s%s", base, icon_name, has_png_ext ? "" : ".png");
        if (n > 0) {
            size_t len = (size_t)n + 1;
            char *path = malloc(len);
            if (path != NULL) {
                snprintf(path, len, "%s/%s%s", base, icon_name, has_png_ext ? "" : ".png");
                if (sni_path_is_regular_file_limited(path)) {
                    return path;
                }
                free(path);
...
static char *sni_icon_resolve_png_path(const char *icon_name, const char *icon_theme_path) {
...
static struct wlr_buffer *sni_icon_buffer_from_png_path(const char *path) {
    if (!sni_path_is_regular_file_limited(path)) {
        return NULL;
    }
...
static void sni_item_request_all(struct fbwl_sni_item *item) {
    if (item == NULL) {
        return;
    }
...
static void sni_item_subscribe(struct fbwl_sni_item *item) {
    if (item == NULL || item->watcher == NULL || item->watcher->bus == NULL) {
        return;
    }
...
static int sni_method_register_item(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
    ...
    if (existing == NULL) {
        ...
        if (watcher->server != NULL) {
            server_toolbar_ui_rebuild(watcher->server);
        }
...
static int sni_name_owner_changed(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
    ...
    if (changed && watcher->server != NULL) {
        server_toolbar_ui_rebuild(watcher->server);
    }
...
static bool server_sni_start(struct fbwl_server *server, struct wl_event_loop *loop) {
    ...
    struct fbwl_sni_watcher *watcher = &server->sni;
...
#ifdef HAVE_SYSTEMD
    (void)server_sni_start(&server, loop);
#endif
