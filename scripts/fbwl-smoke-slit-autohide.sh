...
SOCKET="${SOCKET:-wayland-fbwl-slit-autohide-$UID-$$}"
LOG="${LOG:-/tmp/fluxbox-wayland-slit-autohide-$UID-$$.log}"
CFGDIR="$(mktemp -d "/tmp/fbwl-slit-autohide-$UID-XXXXXX")"
...
timeout 5 bash -c "until rg -q 'Init: slit .* autoHide=1' '$LOG'; do sleep 0.05; done"
...
if [[ "$layout" =~ edge=([^[:space:]]+) ]]; then
  EDGE="${BASH_REMATCH[1]}"
else
  echo "failed to parse slit layout edge: $layout" >&2
  exit 1
fi

if [[ "$layout" =~ x=([-0-9]+)[[:space:]]y=([-0-9]+)[[:space:]]w=([0-9]+)[[:space:]]h=([0-9]+) ]]; then
  SLIT_X="${BASH_REMATCH[1]}"
  SLIT_Y="${BASH_REMATCH[2]}"
  SLIT_W="${BASH_REMATCH[3]}"
  SLIT_H="${BASH_REMATCH[4]}"
else
  echo "failed to parse slit layout box: $layout" >&2
  exit 1
fi

case "$EDGE" in
  right)
    SHOW_X=$((SLIT_X + SLIT_W - 1))
    SHOW_Y=$((SLIT_Y + 1))
    ;;
  left)
    SHOW_X=$((SLIT_X + 1))
  ...
  top)
    ...
    ;;
  *)
    echo "unexpected slit edge=$EDGE in layout: $layout" >&2
    exit 1
    ;;
esac

# Enter the slit first to establish hovered=true. autoHide only hides after a hover transition.
./fbwl-input-injector --socket "$SOCKET" motion "$((ITEM_X + 2))" "$((ITEM_Y + 2))" >/dev/null 2>&1 || true

# Leave the slit; it should hide after session.autoRaiseDelay.
OFFSET=$(wc -c <"$LOG" | tr -d ' ')
./fbwl-input-injector --socket "$SOCKET" motion 0 0 >/dev/null 2>&1 || true
START=$((OFFSET + 1))
timeout 5 bash -c "until tail -c +$START '$LOG' | rg -q 'Slit: autoHide hide'; do sleep 0.05; done"

# Move to the "peek" edge region; slit should show immediately on hover.
OFFSET=$(wc -c <"$LOG" | tr -d ' ')
./fbwl-input-injector --socket "$SOCKET" motion "$SHOW_X" "$SHOW_Y" >/dev/null 2>&1 || true
START=$((OFFSET + 1))
timeout 5 bash -c "until tail -c +$START '$LOG' | rg -q 'Slit: autoHide show'; do sleep 0.05; done"

echo "ok: slit autoHide parity smoke passed (socket=$SOCKET log=$LOG)"
