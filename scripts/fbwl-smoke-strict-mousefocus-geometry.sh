...
LOG="${LOG:-/tmp/fluxbox-wayland-strict-mousefocus-geometry-$UID-$$.log}"
CFGDIR="$(mktemp -d "/tmp/fbwl-strict-mousefocus-geometry-$UID-XXXXXX")"
...
# Maximize the focused window (geo-b).
...
# Move the cursor onto the (still-mapped) geo-a position while geo-b is maximized (so the pointer remains inside geo-b).
./fbwl-input-injector --socket "$SOCKET" motion "$POS_AX" "$POS_AY" >/dev/null 2>&1 || true

# StrictMouseFocus should shift focus on geometry-only changes (unmaximize) even without pointer motion.
OFFSET=$(wc -c <"$LOG" | tr -d ' ')
./fbwl-input-injector --socket "$SOCKET" key alt-f2 >/dev/null 2>&1
START=$((OFFSET + 1))
timeout 2 bash -c "until tail -c +$START '$LOG' | rg -q 'Maximize: geo-b off'; do sleep 0.05; done"
timeout 2 bash -c "until tail -c +$START '$LOG' | rg -q 'Focus: geo-a'; do sleep 0.05; done"

# StrictMouseFocus should shift focus on geometry-only moves (MoveTo) even without pointer motion.
./fbwl-smoke-client --socket "$SOCKET" --title geo-c --stay-ms 10000 >/dev/null 2>&1 &
C_PID=$!
timeout 5 bash -c "until rg -q 'Place: geo-c ' '$LOG'; do sleep 0.05; done"
timeout 5 bash -c "until rg -q 'Focus: geo-c' '$LOG'; do sleep 0.05; done"

OFFSET=$(wc -c <"$LOG" | tr -d ' ')
./fbwl-input-injector --socket "$SOCKET" key alt-3 >/dev/null 2>&1
START=$((OFFSET + 1))
timeout 2 bash -c "until tail -c +$START '$LOG' | rg -q 'MoveTo: geo-c '; do sleep 0.05; done"
timeout 2 bash -c "until tail -c +$START '$LOG' | rg -q 'Focus: geo-a'; do sleep 0.05; done"

echo "ok: strict-mousefocus geometry smoke passed (socket=$SOCKET log=$LOG)"
