...
WAYLAND_DISPLAY="$SOCKET" sh ./util/fbsetbg "$WALLPAPER_PNG"
timeout 5 bash -c "until rg -q 'Background: wallpaper set' '$LOG'; do sleep 0.05; done"

if ! ./fbwl-screencopy-client --socket "$SOCKET" --timeout-ms 4000 --expect-rgb '#00ff00' --sample-x 100 --sample-y 100 >"$SC_LOG" 2>&1; then
...
echo "ok: fbsetbg-wayland smoke passed (socket=$SOCKET log=$LOG png=$WALLPAPER_PNG)"
