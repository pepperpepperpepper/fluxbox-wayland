...
LOG="${LOG:-/tmp/fluxbox-wayland-tray-iconname-$UID-$$.log}"
ICON_NAME="${ICON_NAME:-fbwl-test-icon}"
...
dbus-run-session -- bash -c '
  ...
  mkdir -p "$THEME_ROOT/share/icons/hicolor/16x16/status"
  base64 -d > "$THEME_ROOT/share/icons/hicolor/16x16/status/$ICON_NAME.png" <<'"'"'EOF'"'"'
iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAFklEQVR4nGNg+E8hHDVg1IBRA4aLAQAxtf4QMNSIagAAAABJRU5ErkJggg==
EOF

  : >"$LOG"
  XDG_DATA_HOME="$THEME_ROOT/share" XDG_DATA_DIRS="$THEME_ROOT/share" \
  ...
  echo "ok: tray icon-name smoke passed (socket=$SOCKET log=$LOG)"
'
