...
run_case() {
  (
    ...
    local SOCKET="wayland-fbwl-mousebind-toolbar-$UID-$$-$NAME"
    local LOG="/tmp/fluxbox-wayland-mousebind-toolbar-$UID-$$-$NAME.log"
    local CFG_DIR
    CFG_DIR="$(mktemp -d "/tmp/fbwl-mousebind-toolbar-$NAME-XXXXXX")"

    local MARK_DEFAULT="/tmp/fbwl-toolbar-default-$UID-$$-$NAME"
    local MARK_BIND="/tmp/fbwl-toolbar-bind-$UID-$$-$NAME"
...
OnToolbar Click1 :ExecCommand touch '$MARK_BIND'
EOF
    else
      : >"$CFG_DIR/keys"
    fi
    ...
    local CLICK_X=$((TB_X + LX + W / 2))
    local CLICK_Y=$((TB_Y + TB_H / 2))
    if (( CLICK_X < TB_X || CLICK_X >= TB_X + TB_W )); then
      echo "computed click X out of toolbar bounds: click_x=$CLICK_X toolbar_x=$TB_X w=$TB_W (case=$NAME log=$LOG)" >&2
      exit 1
    fi
    ...
    if [[ "$WITH_BINDING" == "1" ]]; then
      ...
      if [[ -f "$MARK_DEFAULT" ]]; then
        echo "unexpected: toolbar default command ran even though OnToolbar Click1 binding exists (case=$NAME log=$LOG)" >&2
        exit 1
      fi
      if tail -c +$((OFFSET + 1)) "$LOG" | rg -qF 'Toolbar: click button.smoke '; then
        echo "unexpected: toolbar click handler ran even though OnToolbar binding should preempt it (case=$NAME log=$LOG)" >&2
        exit 1
      fi
    else
      timeout 2 bash -c "until [[ -f '$MARK_DEFAULT' ]]; do sleep 0.05; done"
      timeout 2 bash -c "until tail -c +$((OFFSET + 1)) '$LOG' | rg -qF 'Toolbar: click button.smoke '; do sleep 0.05; done"
      if [[ -f "$MARK_BIND" ]]; then
        echo "unexpected: OnToolbar binding ran in no-binding case (case=$NAME log=$LOG)" >&2
        exit 1
      fi
    fi

    echo "ok: toolbar mousebinding precedence case passed (name=$NAME binding=$WITH_BINDING socket=$SOCKET log=$LOG)"
  )
}
...
echo "ok: OnToolbar mousebinding precedence smoke passed"
