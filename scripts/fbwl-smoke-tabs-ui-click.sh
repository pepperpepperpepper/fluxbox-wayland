...
if [[ "$TAB0_ACTIVE" == "1" ]]; then
  CLICK_IDX=1
  CLICK_TITLE="$t1"
  CLICK_X=$((TAB1_LX + TAB1_W / 2))
  CLICK_Y=$((TAB1_LY + TAB1_H / 2))
fi

OFFSET=$(wc -c <"$LOG" | tr -d ' ')
./fbwl-input-injector --socket "$SOCKET" click "$CLICK_X" "$CLICK_Y" >/dev/null 2>&1
tail -c +$((OFFSET + 1)) "$LOG" | rg -q "TabsUI: click idx=$CLICK_IDX title=$CLICK_TITLE"
tail -c +$((OFFSET + 1)) "$LOG" | rg -q "Tabs: activate reason=tab-click title=$CLICK_TITLE"

echo "ok: tabs UI click-to-activate passed"
