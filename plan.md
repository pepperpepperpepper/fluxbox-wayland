...
- [ ] Implement a general Fluxbox-style resource loader for `init`:
  - Parse and store arbitrary keys (not hardcoded handfuls) and provide typed getters (bool/int/string/color).
  - Support Fluxbox-compatible path resolution and file discovery (`keyFile/appsFile/menuFile/styleFile/styleOverlay/slitlistFile/...`).
  - Keep `--disable-x11 --enable-wayland` viable (avoid pulling in Xrm/X11 libs for config parsing).
